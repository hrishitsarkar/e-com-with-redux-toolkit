...
const ProductCard = ({ product }) => {
    ...
    return (
        <div className="flex flex-col  shadow-2xl hover:shadow-2xl hover:shadow-indigo-500/40 w-[300px] items-center p-2 m-2">
            ...
            <button onClick={() => isLoggedIn && addToCart(product)} className="w-[80%] bg-facebook-blue text-white m-2 h-[50px] rounded-lg font-bold">{isLoading ? <div className="flex items-center justify-center"><ClipLoader color="#36d7b7" /><span className="font-bold">Adding...</span></div> : "Add to Cart"}</button>
        </div>);
}
